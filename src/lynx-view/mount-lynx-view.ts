/**
 * Mount a Lynx-for-Web bundle in a `<lynx-view>`, with no UI framework.
 *
 * This is the half of `<WebIframe>` that is not React: creating the element,
 * ordering `browserConfig` before `url`, the fit/responsive viewport decision,
 * and working out whether the page has actually painted. It owns a subtree of
 * plain elements and reports state through a callback, so a React, Preact,
 * Vue, Svelte or Octane site — or a plain script — can all drive it.
 *
 * The scale maths and the mode decision are reused from
 * `../example-preview/utils`, not copied, so there is one implementation.
 *
 * The three things this exists to get right, none of which are obvious:
 *
 *   1. `browserConfig` is a *property* and must be assigned before `url`, or
 *      the page renders against the wrong pixel ratio.
 *   2. An example is authored for a phone viewport. Below a threshold the view
 *      keeps its design size and is transformed down (`fit`); above it, the
 *      view takes the container and `rpx`/`vw`/`vh` are re-based onto it
 *      (`responsive`). Crossing that boundary uses hysteresis, or a resize
 *      drags the layout back and forth across it.
 *   3. There is no public "rendered" event. The page root appearing inside the
 *      shadow tree is the signal, and it has to ignore the disposed root a
...
import {
  ...
  computeScaleRange,
  lerpFitScale,
} from '../example-preview/utils/fit-scale';
import { isFinitePositive } from '../example-preview/utils/number';
import {
  resolveWebPreviewModeWithHysteresis,
  type ResolvedWebPreviewFitKind,
  type ResolvedWebPreviewMode,
  type WebPreviewMode,
} from '../example-preview/utils/resolve-web-preview';
import {
  playAutoGesture,
...
export type LynxViewState = {
  ...
  error: string | null;
  /**
   * The bundle has been fetched, so a rebuild does not go back to the network
   * and a refresh control is worth offering. Same fact as `WebIframe`'s
   * `onCanRefreshChange`.
   */
  canRefresh: boolean;
};

export type LynxViewOptions = {
  /** URL of the `.web.bundle` to mount. */
  url: string;
  /**
   * Resolves once `<lynx-view>` is defined. Injected rather than chosen here,
   * so a host can import `@lynx-js/web-core/client` however it bundles, or
   * load the prebuilt client from a URL.
   */
  ...
  /**
   * Rebuild the page once a resize has settled, so an example that derives
   * anything from `SystemInfo` is re-seeded with the new viewport. On by
   * default; see `scheduleViewportResync`.
   */
  resyncOnResize?: boolean;
  /** Demonstrate a gesture-driven example with simulated touches. */
  autoGesture?: AutoGestureOptions;
  onStateChange?: (state: LynxViewState) => void;
};

export type LynxViewHandle = {
  /** Rebuild the page from the bundle already in memory. */
  reload: () => void;
  dispose: () => void;
};
 ...
 */
export function mountLynxView(
  container: HTMLElement,
  options: LynxViewOptions,
): LynxViewHandle {
  ...
  // The same subtree `WebIframe` renders: a centred flex box, an anchor the
  // fit path positions against, and the frame that carries the transform.
  ...
  function applyLayout(): void {
    ...
    const animate =
      hasLaidOut && resolvedMode === 'fit' && resolved.mode === 'fit';
    ...
    if (resolved.mode === 'responsive') {
      ...
      // `cq*` units resolve against the view's own box, so `rpx`, `vw` and
      // `vh` inside the Lynx page track the panel.
      applyViewStyle({
        width: '100%',
        height: '100%',
        'container-type': 'size',
   ...
   * `cssText` replaces the whole attribute, so a resize would silently take
   * that `display` away and collapse the preview to zero width.
   */
  ...
  function fitScale(
    ...
    if (
      !isFinitePositive(containerWidth) ||
      !isFinitePositive(containerHeight)
    ) {
      return 0;
    }
    ...
    if (!Number.isFinite(range.contain) || !isFinitePositive(range.cover)) {
      return 0;
    }
    ...
    const floor = Math.max(
      minReadableWidth / designWidth,
      minReadableHeight / designHeight,
    );
    if (
      (containerWidth < minReadableWidth ||
        containerHeight < minReadableHeight) &&
      scale < floor
    ) {
  ...
  function autoFitBiases(
    ...
    if (settings.webPreviewMode !== 'auto' || settings.fit !== 'auto') {
      return none;
    }
    ...
    if (resolved.fitKind === 'width') {
      ...
      // Shape the width-driven response as cover plateau -> contain window ->
      // cover plateau, so the panel neither letterboxes early nor crops on the
      // way down.
      const narrowness = clamp01((enterThresholdScale - ratioW) / range);
      const leftCover = 1 - smoothstep01((narrowness - 0.18) / 0.48);
      const midContain =
        smoothstep01((narrowness - 0.4) / 0.14) *
        (1 - smoothstep01((narrowness - 0.62) / 0.14));
   ...
   * later assignment does not re-lay-out an already-rendered page.
   */
   ...
   *
   * Because `browserConfig` is read once and a later assignment does not
   * re-lay-out, after a resize the Lynx page is still laid out for the old
   * viewport: an example that derives anything from `SystemInfo` — a carousel's
   * slide width, a waterfall's column width — keeps the stale value and drifts
   * out of alignment with the box it is drawn in. Rebuilding re-seeds it, and
   * the bundle is cached, so it costs a rebuild rather than a download.
   *
   * Debounced, or dragging a split pane would restart the example every frame.
   */
  function scheduleViewportResync(): void {
    if (disposed || !view || !settings.resyncOnResize || builtWidth === 0) {
      return;
    }
    const settled = () =>
    ...
    resyncTimer = setTimeout(() => {
      if (disposed || !view || settled()) return;
      handle.reload();
    }, VIEWPORT_RESYNC_MS);
  }
  ...
  function fail(reason: unknown): void {
    publish({
      error: formatError(reason) ?? 'The Lynx runtime reported an error.',
    });
  }
   ...
   * first match therefore never observes the rebuilt page, and a refresh can
   * only ever end on the fallback timeout.
   */
  function pageRoot(): Element | null {
    const roots = view?.shadowRoot?.querySelectorAll(
      '[part="page"], [lynx-tag="page"]',
    );
    // Array.from, not spread: the package's tsconfig lib has DOM without
    // DOM.Iterable, so a NodeList is not iterable here.
    const live = roots ? Array.from(roots) : [];
  ...
  function hasDecodedTemplate(shadow: ShadowRoot): boolean {
    return Array.from(shadow.querySelectorAll('style')).some(
  ...
  function evaluateShadow(shadow: ShadowRoot): void {
    ...
    // something a new root could be confused with.
    ...
    // Either signal means the bundle itself has arrived.
  ...
  function startView(): void {
    ...
    view.addEventListener('error', (event: Event) => {
      const detail = (
        event as CustomEvent<{ error?: unknown; message?: unknown }>
      ).detail;
  ...
  /** Only after the page has painted: a gesture into a blank view shows nothing. */
  ...
  const handle: LynxViewHandle = {
     ...
     * Rebuild the page.
     *
     * `<lynx-view>.reload()` looks like the way to do this, but on
     * `@lynx-js/web-core@0.22.2` it tears the page down without building a new
     * one: the root keeps its identity, gains `l-disposed`, and nothing
     * replaces it — so the preview is left showing a dead tree until the
     * fallback timeout gives up. A fresh element rebuilds deterministically,
     * and the bundle comes from the HTTP cache, so this costs a rebuild rather
     * than a download.
     */
    reload(): void {
      if (disposed) return;
